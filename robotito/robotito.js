    ...
    switch (commandName) {
        ...
        case 'verificar':
            { // Bloque para el nuevo comando de verificación
                ...
                const userUrlMatch = url.match(/truckersmp\.com\/(?:user|profile)\/(\d+)/);

                if (!userUrlMatch || !userUrlMatch[1]) {
                    await interaction.editReply({ content: 'La URL proporcionada no es un enlace de perfil de usuario de TruckersMP válido. Asegúrate de que sea la URL completa.', ephemeral: true });
                    return;
                }
